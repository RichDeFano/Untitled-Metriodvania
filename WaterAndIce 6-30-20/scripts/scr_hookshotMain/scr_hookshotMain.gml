if (!instance_exists(obj_newHS))//if ((global.dodgeDarkUnlocked == false) && (global.dodgeLightUnlocked == false))
{
	if (key_ltrheld)
	{
		...
		ropeLengthComplete = setRopeLength;
		//drawThisLine = true;
		//state = pState.swing;
	...
	{	
		...
		/////
			...
			var q = scr_collision_line_point(x, y, destX, destY, obj_noHookshotWall, true, true);
			var s = scr_collision_line_point(x,y,destX,destY,obj_icyWall,true,true);
			if (((r[1] == destX) && (r[2] == destY)) || (failedShot == true) || (r[1] == q[1]) && (r[2] == q[2]) || (r[1] == s[1]) && (r[2] == s[2]))
			{
			...
			else
			{
				...
				hook = instance_create_layer(x,y,"ObjOverPlayer",obj_newHS);
				hook.ropeMaxLength = point_distance(x,y,r[1],r[2]);
				hook.ropeAngle = ropeAngle;
				hook.ropeLength = 0;
